...
export class BitcoinProvider {
  ...
  constructor(baseUrl?: string) {
    this.baseUrl =
      baseUrl || process.env.BITCOIN_API_URL || "https://blockstream.info/api";
  ...
  async getHistory(
    ...
    try {
      ...
      const transactions: Transaction[] = response.data.map((tx: any) => {
        ...
        const relevantInput = tx.vin.find(
      ...
      return {
        ...
        error: {
          ...
          message:
            error.message || "Failed to fetch Bitcoin transaction history",
          details: error.response?.data,
        },
        timestamp: Date.now(),
      };
