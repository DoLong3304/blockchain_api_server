...
export class CoinGeckoProvider {
  ...
  async getMultiplePrices(
    ...
    try {
      const coinIds = assetIds.map((id) => this.getCoinGeckoId(id)).join(",");
      const response = await this.client.get("/simple/price", {
        params: {
          ids: coinIds,
          vs_currencies: currency,
   ...
   */
  private async getERC20PriceByContract(
    assetId: string,
    currency: string
  ): Promise<ApiResponse<PriceResponse>> {
    try {
      ...
      if (!data) {
        throw new Error(
          `Price data not found for ERC20 token ${contractAddress} on ${platform}`
        );
      }
      const priceData: PriceResponse = {
        price: data[currency] || 0,
  ...
  private getCoinGeckoId(assetId: string): string {
    ...
    if (assetId.includes("/")) {
      ...
      if (chainNamespace === "bip122") {
        return "bitcoin";
      } else if (chainNamespace === "eip155" && assetNamespace === "slip44") {
