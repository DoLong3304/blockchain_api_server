...
export class AlchemyProvider {
  ...
  constructor(apiKey?: string) {
    ...
    this.client = axios.create({
      baseURL:
        process.env.ALCHEMY_BASE_URL || "https://ethmainnet.g.alchemy.com/v2",
      timeout: 30000,
      headers: {
      ...
      if (!this.apiKey) {
        return {
          ...
          error: {
            ...
            message:
              "Alchemy API key is required. Please set ALCHEMY_API_KEY environment variable.",
            details: null,
          },
          timestamp: Date.now(),
        };
  ...
  async getGas(
    networkId: string,
    type: "legacy" | "eip1559" = "eip1559"
  ): Promise<ApiResponse<GasPriceResponse>> {
    try {
      if (!this.apiKey) {
        return {
          success: false,
  ...
  async getHistory(
    ...
    try {
      ...
      if (!alchemyNetwork) {
        throw new Error(
          `Alchemy not supported for network: ${assetInfo.networkId}`
        );
      }
      ...
      const transactions: Transaction[] = transfers.map((transfer: any) => ({
        ...
        valueFormatted: transfer.value
          ? weiToEther(transfer.value.toString())
          : "0",
  ...
  async getNftOwners(
    ...
    networkId: string,
    tokenId?: string
  ): Promise<ApiResponse<NftOwnerResponse>> {
    try {
      const alchemyNetwork = ALCHEMY_NETWORKS[networkId];
      if (!alchemyNetwork) {
...
${networkId}`);
      ...
      const restUrl = `https://${alchemyNetwork}.g.alchemy.com/nft/v3/${this.apiKey}/getOwnersF
orCollection`;
      ...
      for (const item of ownerAddresses) {
        ...
        if (Array.isArray(balances) && balances.length > 0) {
          ...
          nftOwners.push({ owner: ownerAddress, tokenId: "", balance: "1" });
        }
      }
      const ownerResponse: NftOwnerResponse = {
        owners: nftOwners,
...
${networkId}`);
      ...
      const restUrl = `https://${alchemyNetwork}.g.alchemy.com/nft/v3/${this.apiKey}/getNFTsFor
Owner`;
      const params: Record<string, string | string[]> = { owner: ownerAddress };
      if (contractAddress) {
  ...
  async getNftMetadata(
    ...
    try {
      ...
      if (!assetInfo.contractAddress || !assetInfo.tokenId) {
        throw new Error(
          "Contract address and token ID are required for NFT metadata"
        );
      }
      const alchemyNetwork = ALCHEMY_NETWORKS[assetInfo.networkId];
      if (!alchemyNetwork) {
...
${networkId}`);
      ...
      return {
        ...
        error: {
          ...
          message: error.message || "Failed to fetch NFT collectionmetadata",
          details: error.response?.data,
        },
        timestamp: Date.now(),
      };
