...
export class BlockchainService {
  ...
  async getPriceHistory(
    ...
    try {
      return await this.coingeckoProvider.getPriceHistory(
        assetId,
        days,
        currency
      );
  ...
  async getHistory(
    ...
    try {
      const assetInfo = parseAssetId(assetId);
      if (assetInfo.type === "bitcoin") {
        return await this.bitcoinProvider.getHistory(address, page, limit);
      } else {
        return await this.alchemyProvider.getHistory(
          address,
          assetId,
          page,
          limit
  ...
  async getNftOwners(
    ...
    try {
      ...
      return await this.alchemyProvider.getNftOwners(
        contractAddress,
        networkId,
        tokenId
      );
  ...
  async getNftsForOwner(
    ...
    try {
      ...
      return await this.alchemyProvider.getNftsForOwner(
        owner,
        networkId,
        contractAddress
      );
  ...
  async getPortfolioSummary(
    ...
    try {
      ...
      return {
        success: true,
        data: { totalValue, currency, portfolio, timestamp: Date.now() },
        timestamp: Date.now(),
      };
    } catch (error: any) {
      return {
